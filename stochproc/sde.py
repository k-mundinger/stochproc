...
class OneDimSDE(Process):
    ...
    def get_trajectory(self):
        ...
        dW_t = np.random.normal(0, np.sqrt(self.dt), size=self.num_steps)
        ...
        for i in range(1, self.num_steps):
            ...
            trajectory[i] = (
                ...
                self.sigma(trajectory[i - 1], self.timeaxis[i - 1]) *
                dW_t[i - 1])
    ...
    def get_increments(self):
        ...
        for i in range(1, self.num_steps):
            ...
            increments[i] = (
                self.b(increments[i - 1], self.timeaxis[i - 1]) * self.dt +
                self.sigma(increments[i - 1], self.timeaxis[i - 1]) *
                dW_t[i - 1])
